...
public interface UserDaoInterface extends JpaRepository<User, String> {
	...
	/**
	 * /** Checks if the email exists already or not, if the email exists throws
	 * exception, if no result found then null is returned.
	 *
	 * @param email: The email id of user.
	...
	/**
	 * Deactivates user. Find {@link User} by uuid, if user exists then deactivates
	 * user.
	 *
	 * @param uuid The id of user.
	 */
